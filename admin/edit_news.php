...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['news_update'])) {
    ...
    if ($title === '' || $content === '') {
        ...
        // Secure image upload (same logic as save_news.php)
        if (isset($_FILES['image']) && is_uploaded_file($_FILES['image']['tmp_name']) && ($_FILES['image']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
            ...
            if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
                error_log('edit_news.php: unable to create upload directory: ' . $uploadDir);
                ...
                if ($fileSize <= 0 || $fileSize > $maxFileSize) {
                    error_log('edit_news.php: uploaded file size invalid or too large. size=' . $fileSize . ' user=' . ($_SESSION['admin_username'] ?? 'unknown'));
                    ...
                    if ($imageInfo === false || $detectedExt === null) {
                        error_log('edit_news.php: uploaded file failed image validation. mime=' . ($mime ?? 'unknown') . ' getimagesize=' . var_export($imageInfo, true) . ' orig=' . $origName . ' user=' . ($_SESSION['admin_username'] ?? 'unknown'));
                        $error = 'Tệp tải lên không hợp lệ. Vui lòng chọn file ảnh (jpg, png, webp).';
                    } else {
                        $useExt = $detectedExt;

                        if (!in_array($useExt, $allowedExt, true)) {
                            error_log('edit_news.php: detected extension not allowed: ' . $useExt . ' mime=' . $mime . ' orig=' . $origName);
                            ...
                            if (move_uploaded_file($tmpPath, $targetFile)) {
                                ...
                                error_log('edit_news.php: move_uploaded_file failed. tmp=' . $tmpPath . ' target=' . $targetFile . ' user=' . ($_SESSION['admin_username'] ?? 'unknown'));
